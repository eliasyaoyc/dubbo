...
/**
 * ConditionRouter
 * 条件路由  host = 10.20.153.10 => host = 10.20.153.11
 */
public class ConditionRouter extends AbstractRouter {
    ...
    /**
     * 对服务提供者和消费者规则进行解析
     * 逻辑就是：由正则表达式和一个while循环以及数个条件分钟组成。
     */
    private static Map<String, MatchPair> parseRule(String rule)
        ...
        while (matcher.find()) { // Try to match one by one
            ...
            //分隔符为 ！=
                ...
                //将content 存入到MatcherPair的 mismatches集合中
                values.add(content);
            }
    ...
    boolean matchWhen(URL url, Invocation invocation) {
        ...
        return whenCondition == null || whenCondition.isEmpty() ||
                //whenCondition是服务消费者匹配条件,第二个参数url源自route方法的参数列表，该参数由外部类调用route方法时传入
                matchCondition(whenCondition, url, null, invocation);//条件匹配
    }

    private boolean matchThen(URL url, URL param) {
        // 服务提供者条件为 null 或空，表示禁用服务
        return !(thenCondition == null || thenCondition.isEmpty()) &&
                //第二个参数和第三个参数 分别是服务提供者的url 和服务消费者的url。
    ...
    //内部类  包含两个set类型的成员变量，分别用于存放匹配和不匹配的条件
